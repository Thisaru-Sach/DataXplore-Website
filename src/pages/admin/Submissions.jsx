...
export default function Submissions() {
  ...
  async function load() {
    ...
    setError("");
    try {
      setSubs(await getAllSubmissions(stage));
    } catch (e) {
      setError(e.message);
    } finally {
      setLoading(false);
  ...
  async function remove(sub) {
    ...
    try {
      await deleteSubmission(sub.id, sub.file_path);
      await load();
    } catch (e) {
      alert(e.message);
    }
  ...
  return (
    <>
      <div className="adm-topbar">
        <h1>
          Submissions <span className="adm-count">{subs.length}</span>
        </h1>
            ...
            <tbody>
              {filtered.map((s) => (
                <tr key={s.id}>
                  ...
                  <td>
                    <strong>{s.teams?.team_name ?? "—"}</strong>
                  </td>
                  ...
                  <td>
                    <div className="adm-action-row">
                      ...
                      <button className="adm-del-btn" onClick={() => remove(s)}>
                        🗑
                      </button>
                    </div>
                  </td>
